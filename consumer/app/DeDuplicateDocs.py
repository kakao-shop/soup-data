...
class DeDuplicate:
    def __init__(self):
        self.es = Elasticsearch(["192.168.56.101:9200","192.168.56.102:9200","192.168.56.103:9200"])
        self.dict_of_duplicate_docs = {}
        # 다음 줄은 문서의 중복 여부를
        # 판단하는 데 사용될 필드를 정의합니다.
        self.keys_to_include_in_hash = ["name"]
        ...
        def scroll_over_all_docs():
            data = self.es.search(index="dedup2", scroll='1m',  body={"query": {"match_all": {}},
                                                                "sort":[{"cnt":{"order":"desc"}}]})
            # 스크롤 ID를 가져옵니다.
            sid = data['_scroll_id']
            scroll_size = len(data['hits']['hits'])
            print("scroll_size", data)
        ...
        def loop_over_hashes_and_remove_duplicates():
            ...
            for hashval, array_of_ids in self.dict_of_duplicate_docs.items():
                if len(array_of_ids) > 1:
                    ...
                    matching_docs = self.es.mget(index="dedup2", doc_type="doc", body={"ids": array_of_ids})
                    print("array_of_ids", array_of_ids)
                    ...
                    for doc in matching_docs['docs'][1:]:
                        ...
                        self.es.delete_by_query(index="dedup2",
                        body = {
                            "query" :{
                                    "bool": {
                                            "must":[
...
def main():
    ...
    dedup.loop_over_hashes_and_remove_duplicates()
    data = dedup.es.search(index="dedup2", scroll='1m',  body={"query": {"match_all": {}}})
    print("End",data)
main()
